...
if (isset($_POST['submit_user'])) {
	...
	if (($fv->emptyfields(array('emp_name' => $emp_name), NULL))) {
		...
		$update = $db->update('employee', array('emp_name' => $emp_name, 'emp_surname' => $emp_surname, 'address' => $address, 'contact1' => $contact1, 'create_date' => $create_on, 'ip_address' => $ip_address, 'department' => $department, 'company_id' => $company_id), array('employee_id' => $load));
		if ($update) {
			$get_row = $db->get_row('employee', array('employee_id' => $load));
