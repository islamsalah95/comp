...
if (isset($_POST['submit_freelancer'])) {
	...
	$freelancer_assigned_company = $freelancer_company;
	...
	elseif (($fv->emptyfields(array('dob' => $dob), NULL))) {
		...
		if ($update) {
			...
			// }
			// $get_row = $db->get_row('employee', array('employee_id' => $load));
            $insert_map = $db->insert('employee_company_map', array('employee_id' => $load, 'company_id' => $freelancer_assigned_company));
            ...
            echo "<script>
                 setTimeout(function(){
	    		  window.location = '" . $link->link("registered_freelancers", frontend) . "'
	                },2000);</script>";
		}
	}
