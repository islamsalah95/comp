...
if ($_SESSION['department'] == 5 || $_SESSION['department'] == 1 || $_SESSION['department'] == 4 || $_SESSION['department'] == 6) {
	...
	$users = myQuery("SELECT employee_id, emp_name, emp_surname, email from `employee` where ((`department`='2' AND `company_id` ='" . $_SESSION['company_id'] . "') OR (`department`='3' AND `company_id` in (" . implode(',', $user_companies) . ")) ) AND `status` = 0 ");
...
if (isset($_REQUEST['employee_id']) && $_REQUEST['employee_id'] != '') {
	...
	$messages = myQuery("SELECT * from `messages` where `employee_id`= '" . $_REQUEST['employee_id'] . "' AND `company_id` = '" . $_SESSION['company_id'] . "' ");
