...
if (isset($_REQUEST['edit'])) {
    ...
    $manager_companies = $db->get('employee_company_map', array('employee_id' => $load));
    if (!empty($manager_companies)) {
        foreach ($manager_companies as $c) {
            $get_row['company'][] = $c['company_id'];
        }
    }
}
...
if (isset($_POST['submit_user'])) {
    ...
    if (($fv->emptyfields(array('emp_name' => $emp_name), NULL))) {
        ...
        $update = $db->update('employee', array('emp_name' => $emp_name, 'emp_surname' => $emp_surname, 'address' => $address, 'contact1' => $contact1, 'create_date' => $create_on, 'ip_address' => $ip_address, 'department' => $department), array('employee_id' => $load));
        if ($update) {
            ...
            if (!empty($assigned_companies)) {
                foreach ($assigned_companies as $c_id) {
                    $insert_map = $db->insert('employee_company_map', array('employee_id' => $load, 'company_id' => $c_id));
                }
            }
