...
function get_report_hours($employee_id, $start_date, $end_date,$department=null) {
  $report_details = myQuery("SELECT e.employee_id, e.emp_name, sum(sc.check_out_time) as working_hours 
            FROM `shift_check` sc 
...
$report= get_report_hours($contracts[0]['employee_id'], $contracts[0]['start_date'], $contracts[0]['end_date']);
