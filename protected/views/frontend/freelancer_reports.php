...
<div id="content-container">
  <div class="pageheader">
    <h3><i class="fa fa-home"></i> <?php echo ucfirst($company_details['company_name']); ?> - <?php echo $lang[$r_t]; ?> <?php echo $lang['employee'] . ' ' . $lang['report']; ?></h3>
    <div class="breadcrumb-wrapper">
      <span class="label"><?php echo $lang['you_are_here']; ?>:</span>
      <ol class="breadcrumb">
        <li class="active"><?php echo $lang['employee'] . ' ' . $lang['report']; ?></li>
      </ol>
    </div>
  </div>
  ...
  <div id="page-content">
    <div class="row">
      ...
      <div class="col-3">
    <form method="post" action="<?= $link->link("freelancer_reports", frontend) ?>">
        <input type="hidden" name="twc" value="<?= $is_rt ?>">
        <button type="submit" class="btn btn-primary" <?php if (isset($is_rt) && $is_rt == "twc") { ?>disabled<?php } ?>>
            <?php echo $lang['twc']; ?>
        </button>
    </form>
    </div>
    ...
    <form method="post" action="<?= $link->link("freelancer_reports", frontend) ?>">
        <input type="hidden" name="active" value="<?= $is_rt ?>">
        <button type="submit" class="btn btn-primary" <?php if (isset($is_rt) && $is_rt == "active") { ?>disabled<?php } ?>>
    ...
    <form method="post" action="<?= $link->link("freelancer_reports", frontend) ?>">
        <input type="hidden" name="inactive" value="<?= $is_rt ?>">
        <button type="submit" class="btn btn-primary" <?php if (isset($is_rt) && $is_rt == "inactive") { ?>disabled<?php } ?>>
    ...
    ?>
        <form method="post" action="<?= $link->link("freelancer_reports", frontend) ?>">
            <input type="hidden" name="all_companies" value="1">
            <button type="submit" class="btn btn-primary" <?php if ($all_companies_users == 1) { ?>disabled<?php } ?>>
                <?php echo $lang['all_company']; ?>
            </button>
        </form>
    <?php
    }
